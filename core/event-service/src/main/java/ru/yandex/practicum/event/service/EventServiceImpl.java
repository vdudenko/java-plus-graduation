...
public class EventServiceImpl implements EventService {
    ...
    public EventFullDto createEvent(Long userId, NewEventDto newEventDto) {
        ...
        if (!categoryRepository.existsById(newEventDto.getCategory())) {
            throw new CategoryNotExistException("Category with id=" + newEventDto.getCategory() +
                    " was not found");
        }
        ...
        Event event = eventMapper.toEvent(newEventDto, newEventDto.getCategory(), userId, newEventDto.getLocation());
    ...
    @Override
    @Transactional
    public EventFullDto getEventWithoutStatistic(Long eventId) {
        Event event = eventRepository.findById(eventId)
                .orElseThrow(() -> new NotFoundException("Event with id=" + eventId + " was not found"));
    ...
    @Override
    @Transactional
    public void updateEventConfirmedRequests(Long eventId, Long confirmedCount) {
        Event event = eventRepository.findById(eventId)
                .orElseThrow(() -> new EventNotExistException("Event with id=" + eventId + " was not found"));
    ...
    private void updateEventFieldsFromUserDto(Event event, UpdateEventUserDto dto) {
        ...
        if (dto.getCategory() != null) {
            if (!categoryRepository.existsById(dto.getCategory())) {
                throw new CategoryNotExistException("Category with id=" + dto.getCategory() +
                        " was not found");
            }
    ...
    private void updateEventFieldsFromAdminDTO(Event event, UpdateEventAdminDto dto) {
        ...
        if (categoryId != null) {
            ...
            if (!categoryRepository.existsById(categoryId)) {
                throw new CategoryNotExistException("Category with id=" + categoryId +
                        " was not found");
            }
    ...
    private List<Predicate> buildAdminPredicates(CriteriaBuilder builder, Root<Event> root,
        ...
        if (hasCategories(request.getCategories())) {
            Predicate categoryFilter = root.get("category").in(request.getCategories());
